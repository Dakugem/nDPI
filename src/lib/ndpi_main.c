...
static void init_protocol_defaults(struct ndpi_detection_module_struct *ndpi_str)
{
  ...
  */
  ...
  ndpi_set_proto_defaults(ndpi_str, 1, 1, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_AKAMAI,
                          ...
                          ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */,
                          0);
  ndpi_set_proto_defaults(ndpi_str, 1, 1, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_BIMODAL,
                          "Bimodal", NDPI_PROTOCOL_CATEGORY_NETWORK, NDPI_PROTOCOL_QOE_CATEGORY_UNSPECIFIED,
                          ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                          ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */,
                          0);
  ndpi_set_proto_defaults(ndpi_str, 1, 1, NDPI_PROTOCOL_ACCEPTABLE, NDPI_PROTOCOL_MEESNCP_SIMVIZ,
                          "MEESNCP_SIMVIZ", NDPI_PROTOCOL_CATEGORY_NETWORK, NDPI_PROTOCOL_QOE_CATEGORY_UNSPECIFIED,
                          ndpi_build_default_ports(ports_a, 0, 0, 0, 0, 0) /* TCP */,
                          ndpi_build_default_ports(ports_b, 0, 0, 0, 0, 0) /* UDP */,
                          0);
                          ...
                          FILE *fd, ndpi_protocol_category_t category_id)
{
  ...
  while (1)
  {
    ...
    for (i = 0; i < len; ++i)
    {
      ...
      if (line[i] != '-' && line[i] != '.' && line[i] != ':' && line[i] != '/' && ndpi_isalnum(line[i]) == 0
...
static int dissectors_init(struct ndpi_detection_module_struct *ndpi_str)
{
  ...
  /* BIMODAL */
  init_bimodal_dissector(ndpi_str);

  /* MEESNCP_SIMVIZ*/
  init_meesncp_simviz_dissector(ndpi_str);
...
static u_int8_t ndpi_is_multi_or_broadcast(struct ndpi_flow_struct *flow)
{
  ...
  if (!flow->is_ipv6)
  {
    ...
    if (((daddr & 0xF0000000) == 0xE0000000 /* multicast 224.0.0.0/4 */) || ((daddr & 0x000000FF) == 0x000000FF /* last byte is 0xFF, not super correct, but a good approximation */) || ((daddr & 0x000000FF) == 0x00000000 /* last byte is 0x00, not super correct, but a good approximation */) || (daddr == 0xFFFFFFFF))
      return (1);
  }
  else
  {
  ...
  */
      ...
      flow->confidence != NDPI_CONFIDENCE_MATCH_BY_IP)
  {
    ...
    case NDPI_PROTOCOL_TLS:
      ...
      */
      if ((flow->guessed_protocol_id_by_ip == NDPI_PROTOCOL_MICROSOFT_AZURE) && (ret->proto.master_protocol == NDPI_PROTOCOL_UNKNOWN) && ndpi_str->msteams_cache)
      {
        u_int16_t dummy;
                                  ...
                                  struct ndpi_flow_struct *flow)
{
  ...
  if ((flow->l4_proto == IPPROTO_TCP) && (flow->l4.tcp.cli2srv_tcp_flags & TH_PUSH) && (flow->l4.tcp.srv2cli_tcp_flags & TH_PUSH))
  {
                            ...
                            ndpi_protocol *ret)
{
  ...
  if (flow->detected_protocol_stack[0] == NDPI_PROTOCOL_UNKNOWN && flow->confidence == NDPI_CONFIDENCE_DPI_PARTIAL)
  {
    flow->already_gaveup = 0;
  ...
  */
  ...
  if ((ret->proto.app_protocol < ndpi_struct->proto_defaults_num_allocated) && ndpi_struct->proto_defaults[ret->proto.app_protocol].performIPcheck && (ret->proto.app_protocol != ret->protocol_by_ip))
  {
    /* Handle exceptions */
    bool trigger_risk = false;

    if ((ret->proto.app_protocol != NDPI_PROTOCOL_UNKNOWN) && (ret->protocol_by_ip != NDPI_PROTOCOL_UNKNOWN))
    {
      /*
  ...
  belong to the service organization
    ...
    else if ((ret->proto.app_protocol != NDPI_PROTOCOL_UNKNOWN) && (ret->protocol_by_ip == NDPI_PROTOCOL_UNKNOWN))
    {
      /*
...
void ndpi_fill_protocol_category_and_breed(struct ndpi_detection_module_struct *ndpi_str, struct ndpi_flow_struct *flow,
                                           ndpi_protocol *ret)
{
  if ((ret->proto.master_protocol == NDPI_PROTOCOL_UNKNOWN) && (ret->proto.app_protocol == NDPI_PROTOCOL_UNKNOWN))
    return;
                                                   ...
                                                   const ndpi_protocol *returned_proto)
{
  ...
  */
  ...
  if (is_ntop_protocol(returned_proto))
    return (1);
                                             ...
                                             const ndpi_protocol *ret)
{
  ...
  if (found && (found->proto_idx != NDPI_PROTOCOL_UNKNOWN) && (found->proto_idx != ret->proto.master_protocol) && (found->proto_idx != ret->proto.app_protocol))
  {
    ...
    if (!check_protocol_port_mismatch_exceptions(found, ret))
    {
      ...
      */
      ...
      if (!found)
      {
        ...
        if ((r == NULL) || ((r->proto_idx != ret->proto.app_protocol) && (r->proto_idx != ret->proto.master_protocol)))
        {
...
ret_protocols:
  ...
  if ((!flow->risk_checked) && ((ret.proto.master_protocol != NDPI_PROTOCOL_UNKNOWN) || (ret.proto.app_protocol != NDPI_PROTOCOL_UNKNOWN)))
  {
  ...
  if (!flow->tree_risk_checked)
  {
    ...
    if (ndpi_str->ip_risk && ndpi_str->ip_risk->v4 && packet->iph && ndpi_is_public_ipv4(ntohl(packet->iph->saddr)) && ndpi_is_public_ipv4(ntohl(packet->iph->daddr)))
    {
      struct in_addr addr;
  ...
  if ((ret.proto.app_protocol == NDPI_PROTOCOL_UNKNOWN) && (packet->payload_packet_len > 0) && (flow->packet_counter <= 5))
  {
...
{
  return (((proto.app_protocol == NDPI_PROTOCOL_UNKNOWN) && (proto.master_protocol == NDPI_PROTOCOL_UNKNOWN)) ? true : false);
}
